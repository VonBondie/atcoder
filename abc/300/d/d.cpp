...
void solve() {
  LL N; loadVar(N);
  LL ans = 0;
  Prime<int> prime(sqrt(N /(2 * 2 * 3)));

  const list<int>& prime_list = prime.get();
  VI primes;
  ...
  //     while(l != r) {
  //       int idx = (l + r) / 2;
  //       if(tmp * primes[idx] * primes[idx]<= N) {
  //         l = idx;
  //       } else {
  //         r = idx;
  //       }
  //     }
  //     if(tmp * primes[l] * primes[l]<= N) ans += l+1;
  //   }
  // }
