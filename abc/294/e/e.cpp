...
void solve() {
  LL L; int N1, N2; loadVar(L, N1, N2);
  VLL v1(N1), l1(N1); loadVec(N1, v1, l1);
  VLL v2(N2), l2(N2); loadVec(N2, v2, l2);

  int idx1 = 0, idx2 = 0;
  LL head1 = 0, head2 = 0;
  LL count = 0;
  while(head1 < L || head2 < L) {
    if(head1 == head2) {
      if(v1[idx1] == v2[idx2]) {
        count += min(head1+l1[idx1], head2+l2[idx2]) - max(head1, head2);
      } 
      head1 += l1[idx1++]; head2 += l2[idx2++];
    } else if(head1 < head2) {
      if(v1[idx1] == v2[idx2-1]) {
        count += min(head1+l1[idx1], head2) - max(head1, head2 - l2[idx2-1]);
      }
      head1 += l1[idx1++];
    } else {
      if(v2[idx2] == v1[idx1-1]) {
        count += min(head2+l2[idx2], head1) - max(head2, head1 - l1[idx1-1]);
      }
      head2 += l2[idx2++];
    }
  }
