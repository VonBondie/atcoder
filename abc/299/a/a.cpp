...
void solve() {
  ...
  auto pos_fist_i = S.find('|');
  auto pos_aster = S.find('*', pos_fist_i + 1);
  auto pos_second_i = S.find('|', pos_fist_i + 1);

  if(pos_second_i == string::npos) {
    pos_second_i = 0;
  }

  if(pos_fist_i < pos_aster && pos_aster < pos_second_i) {
    cout << "in" << endl;
  } else { 
    cout << "out" << endl;
  }
