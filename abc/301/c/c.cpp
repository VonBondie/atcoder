...
void solve() {
  ...
  rep(i, S.size()) {
    switch(S[i]) {
      case '@': s_at_count++; break;
      default: s_count[S[i] - 'a']++;
    }

    switch(T[i]) {
      case '@': t_at_count++; break;
      default: t_count[T[i] - 'a']++;
    }
  }
  ...
  rep(i, 26) {
    char c = 'a' + i;
    int gap = abs(s_count[i] - t_count[i]);
    switch(c) {
    case 'a':
    case 't':
    case 'c':
    ...
    case 'r':
      if(gap == 0) break;
      if(s_count[i] > t_count[i]) t_at_count -= gap;
      else s_at_count -= gap;
      break;
    default: if(gap) win = false;
    }
    if(t_at_count < 0 || s_at_count < 0) win = false;
  }
