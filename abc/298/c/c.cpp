...
void solve() {
  ...
  rep(i, Q) {
    ...
    switch(tmp) {
      ...
      {
        int card, box_id;
        cin >> card >> box_id; box_id--;
        boxes[box_id].push_back(card);
        numbers[card].insert(box_id+1);
        break;
      }
      case 2:
      {
        int box_id;
        cin >> box_id; box_id--;
        sort(boxes[box_id].begin(), boxes[box_id].end());
        printVec(boxes[box_id]);
        break;
     }
      case 3:
      {
        int number;
        cin >> number;
        auto itr = numbers[number].begin();
        cout << *(itr); itr++;
        while(itr != numbers[number].end()) {
          cout << " " << *(itr);
          itr++;
        }
        cout << endl;
