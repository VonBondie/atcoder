...
vector<unordered_set<int>> U;
VB flags;
LL total;

LL getTotal(VB& flags, int current) {
  total ++;
  if(total >= 1000000) return total;
  flags[current] = true;
  for(auto next: U[current]) {
...
void solve() {
  ...
  rep(i, m) {
    int u_ = u[i] - 1; int v_ = v[i] - 1;
    U[u_].insert(v_); U[v_].insert(u_);
  }

  flags.clear();
  flags.resize(n, false);
  total = 0;
  LL total = min(getTotal(flags, 0), LL(1000000));
