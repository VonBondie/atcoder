...
const string s_check1 = "HDCS"; 
bool check2() {
  for(auto&& s: S) {
    if(s_check1.find(s[0]) == string::npos) return false;
  }
  return true;
}

const string s_check2 = "A2345676789TJQK"; 
bool check3() {
  for(auto&& s: S) {
    if(s_check2.find(s[1]) == string::npos) return false;
  }
  return true;
}
