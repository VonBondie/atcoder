...
string notationKernel() {
  return "";
} template<class Head, class... Tail> string notationKernel(Head&& head, Tail&&... tail) { string s = "," + to_string(head); return s + notation(std::forward<Tail>(tail)...); }
...
void solve() {
  ...
  rep(i, m) {
    ...
    if(U.same(u[i] - 1, v[i] - 1) || d[u[i] - 1] >= 2 || d[v[i] - 1] >= 2) {
      flag = false;
      break;
    }
    U.merge(u[i] - 1, v[i] - 1);
